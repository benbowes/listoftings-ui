...
export default class Index extends Component {
  ...
  componentWillMount() {
    const getShortCategories = () => {
      const sc = _uniqBy(CATEGORIES.map((c) => c.name.split(" / ")[0]));
      return sc
        .map((c) => ({
          shortName: c,
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        {preferredRegion && <SearchForm region={preferredRegion} />}
            ...
            <ul className="specialities">
              {shortCategories.map((shortCategory) => (
                ...
                >
                  ...
                  >
                    <span className="specialities__link__text">
                      {shortCategory.shortName}
                    </span>
                    <span className="specialities__link__link">
                      +SEARCH LAWYERS
                    </span>
                  </a>
                </li>
