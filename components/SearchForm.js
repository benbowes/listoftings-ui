...
const getBackupInputValue = (inputValue) =>
  inputValue !== "" ? sanitiseInputValue(inputValue) : undefined;
...
export default class Index extends Component {
  ...
  getSearchButton() {
    ...
    if (totalResults !== undefined) {
      return (
        <span className="content--fade-in content--fade-in--fast">
          See {totalResults} lawyer{totalResults === 1 ? "" : "s"}
        </span>
      );
    }
  ...
  handleAutoSuggestChange(newValue) {
    ...
    const foundCategory =
      categories.filter((v) => v.name === newValue)[0] || {};
    ...
    this.fetchTotalResults(foundSlug || getBackupInputValue(newValue), region);
  }
  ...
  handleAutoSuggestSelection(suggestion) {
    ...
    this.fetchTotalResults(suggestion.slug, region);
  }

  handleSubmit(e) {
    const { region, categories, autoSuggestValue, inputValue } = this.state;
    const found =
      categories.filter((v) => v.name === autoSuggestValue.name)[0] || {};
    e.preventDefault();
  ...
  toggleButtonDisabled() {
    ...
    const isIncompleteSearch =
      inputValue !== "" && autoSuggestSlug === undefined;
          ...
          >
            <AutoSuggestContainer
              ...
              changeReporter={(newValue) => {
                this.handleAutoSuggestChange(newValue);
              }}
              selectionChangedReporter={(suggestion) => {
                this.handleAutoSuggestSelection(suggestion);
              }}
            />
