...
export default class LawyersSpecialty extends Component {
  static async getInitialProps({ req, res, query, asPath }) {
    const currentPage =
      get([req, "params.page"]) || asPath.substr(asPath.lastIndexOf("/") + 1);
    return res
      ? {
          canonicalUrl: asPath,
          region: query.region,
      ...
      return (
        <GlobalLayout
          title={`${region.replace(
            /-/g,
            " "
          )} Lawyers specializing in ${category}`}
          region={region}
        ...
        >
          <div className="content--fade-in">
            <h1>{`${region.replace(
              /-/g,
              " "
            )} Lawyers specializing in ${category}`}</h1>
          ...
          {totalResults === 0 && (
            <div>
              <div className="content--fade-in">
                ...
                {region && <SearchForm region={region} />}
              </div>
            </div>
          )}
